#!/usr/bin/env python3
"""
Diagnostic script specifically for out3.png to understand the 5-line structure
"""
...
# Try different gap thresholds to find which gives 5 lines
print("\n" + "=" * 80)
print("Testing different gap thresholds:")
print("=" * 80)

best_threshold_factor = None
for threshold_factor in np.arange(0.2, 0.7, 0.01):
    ...
    # Show details for 5 lines
    if len(lines) == 5:
        best_threshold_factor = threshold_factor
        print(f"\n  ✓ FOUND 5 LINES with threshold factor {threshold_factor:.3f}!")
...
# Now visualize with the threshold that gives 5 lines
if best_threshold_factor is None:
    print("\n⚠️  Could not find exact threshold for 5 lines")
    # Find closest
    results = []
    for threshold_factor in np.arange(0.2, 0.7, 0.01):
        gap_threshold = median_height * threshold_factor
        ...
        if current_line:
            lines.append(current_line)
        results.append((threshold_factor, len(lines)))

    print("\nClosest results:")
    for tf, count in sorted(results, key=lambda x: abs(x[1] - 5))[:5]:
        print(f"  Factor {tf:.3f}: {count} lines (diff: {abs(count-5)})")

    best_threshold_factor = sorted(results, key=lambda x: abs(x[1] - 5))[0][0]
    print(f"\nUsing closest threshold factor: {best_threshold_factor:.3f}")
...
output_path = "diagnostic_out3_lines.png"
