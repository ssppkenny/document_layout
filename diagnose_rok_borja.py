#!/usr/bin/env python3
"""
Diagnose W48 (Börja) and W19 (rök) specifically.
"""
...
def analyze_specific_words():
    ...
    for word_idx, word_name in [(18, "W19 (rök)"), (47, "W48 (Börja)")]:
        ...
        for comp in component_info:
            # Classify as diacritic or letter (using binarization thresholds)
        ...
        # Check merge conditions for letter components
        ...
        if len(letter_comps) >= 2:
            print(f"\nChecking merge conditions for {len(letter_comps)} letter components:")
            for idx_i in range(len(letter_comps)):
                for idx_j in range(idx_i + 1, len(letter_comps)):
                    ...
                    status = "✓ SHOULD MERGE" if should_merge else "✗ WON'T MERGE"
                    print(f"\n  #{c_i['id']} vs #{c_j['id']}: {status}")
                    ...
                    print(f"    v_overlap={v_overlap:.0f} > {overlap_threshold:.1f}? {v_overlap > overlap_threshold}")
                    print(f"    height_ratio={height_ratio:.2f} < {height_ratio_threshold}? {height_ratio < height_ratio_threshold}")

if __name__ == '__main__':
    analyze_specific_words()
